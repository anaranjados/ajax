const innNorma  = 12, accNorma	= 20, bikNorma	= 9;
const emptyCol  = allCol[0];
const listOfCust= document.querySelector('select');
let allCol	= document.querySelectorAll('.account');
//
let idObj	= document.getElementsByName('id');
let custNameObj = document.getElementsByName('cust_name');
let innObj 	= document.getElementsByName('inn');
let addressObj  = document.getElementsByName('address');
...
function getList() {
	...
	add.src = "php/get_list.php?params=" + JSON.stringify(send);
...
// ---- Filling the fields ----
listOfCust.addEventListener('change', ()=> {
	...
	const add = document.createElement("script");
	add.src = "php/filling.php?params=" + JSON.stringify(send);	// result appears after 6 functions above
	document.body.appendChild(add);
});
...
function filling_sub(dataset) {
	...
	*/
	if (dataset.length === 0) {
		...
		for (i in dataset) {
			sect.innerHTML 	  += emptyCol.outerHTML;		// as rows come, so many columns are created
		}
		let custId = [];
...
function account(t, c, idObj, aNu, aNa, bi, bal) {
	...
	if (t === 'update') {
		// if ( !isNaN(aNu)  &&  !isNaN(bik)  &&  !isNaN(balance) ) {	// toString(aNu).length === accNorma   &&  toString(bik).length === bikNorma 
			...
			const add = document.createElement("script");
			add.src = 'php/btn_save_acc.php?params=' +JSON.stringify(send);
			document.body.appendChild(add);
		...
		// if ( !isNaN(aNu)  &&  !isNaN(bik)  &&  !isNaN(balance) ) {	// toString(aNu).length === accNorma   &&  toString(bik).length === bikNorma 
			const send = { id: idObj, acc_number: aNu, acc_name: aNa, bik: bi, balance: bal };
			const add = document.createElement("script");
			add.src = 'php/btn_add_acc.php?params=' +JSON.stringify(send);
			document.body.appendChild(add);
		//}		else {alert('Please fill Account correctly')}
	}
	else if (t === 'del') {
		//if ( toString(aNu).length === accNorma  &&  !isNaN(aNu) ) {
			...
			const add = document.createElement("script");
			add.src = 'php/btn_del_acc.php?params=' +JSON.stringify(send);
			document.body.appendChild(add);
		// }	else {alert('This account does not exist, please enter 20 digits')}
	}
	else { console.log('No button was clicked.')}
	getList();
}
...
function updateActC() {
	...
	if (custNameObj.value !== "" && !isNaN(parseInt(innObj.value)) && innObj.value.length === innNorma) {
		...
		try {
			const add = document.createElement("script");
			add.src = "php/btn_save_cust.php?params=" + JSON.stringify(send);
			document.body.appendChild(add);
			}
...
function addActC() {
	...
	if (custNameObj.value != "" && !isNaN(parseInt(innObj.value)) && innObj.value.length === innNorma) {
		...
		try {
			add.src = "php/btn_new_cust.php?params=" + JSON.stringify(send);
			document.body.appendChild(add);
			}
...
function delActC() {
	...
	try {
		...
		add.src = "php/btn_del_cust.php?params=" + JSON.stringify(send);
		document.body.appendChild(add);
		}
